resource "aws_iam_role" "eks_pod_identity_auth_mode" {
  name               = "${var.cluster_name}-pod-identity-auth-mode-role"
  assume_role_policy = data.aws_iam_policy_document.eks_pod_identity_auth_mode_document.json
}
...
resource "kubectl_manifest" "eks_pod_identity_sa" {
  yaml_body = <<-EOF
    ...
    kind: ServiceAccount
    metadata:
      name: ${var.default_eks_pod_identity_auth_mode_name}
      namespace: ${kubectl_manifest.pod_identity_namespace.name}
  EOF
