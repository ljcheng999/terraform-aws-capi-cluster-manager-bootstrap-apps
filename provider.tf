...
provider "argocd" {
  insecure    = true
  server_addr = "${var.argocd_hostname}:443"
  username    = "admin"
  password    = aws_ssm_parameter.argo_admin_password[0].value
  # password = bcrypt(aws_ssm_parameter.argo_admin_password[0].value)
